...
# time ./github --github-es-url="${ESURL}" --github-debug=0 --github-categories='pull_request' --github-tokens="`cat ./secrets/tokens.encrypted.secret`" --github-stream="${STREAM}" --github-org=lukaszgryglicki --github-repo=test-api 2>&1 | tee run.log
time ./github --github-es-url="${ESURL}" --github-debug=0 --github-categories='issue,pull_request' --github-tokens="`cat ./secrets/tokens.encrypted.secret`" --github-stream="${STREAM}" --github-org=cncf --github-repo=toc 2>&1 | tee run.log
